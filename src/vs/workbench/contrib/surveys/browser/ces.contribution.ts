...
class CESContribution extends Disposable implements IWorkbenchContribution {
	...
	private async promptUser() {
		...
		const message = await this.tasExperimentService?.getTreatment<string>('CESSurveyMessage') ?? nls.localize('cesSurveyQuestion', 'Got a moment to help the Cortex team? Please tell us about your experience with Cortex so far.');
