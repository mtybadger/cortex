...
function getLogger(...tags: string[]) {
    ...
    return {
        ...
        showErrorMessageWithCompilerLogButtonAndChat
    }
}
...
function showErrorMessageWithCompilerLogButtonAndChat(message: string) {
    const res = showErrorMessage(message, 'Ask Cortex', 'Open compiler log')
    if (res) {
        return res.then(option => {
            switch (option) {
                case 'Ask Cortex': {
...
function addLogToCortex(){
    COMPILER_PANEL.show()
    setTimeout(() => {
        let compilerPanel: vscode.TextEditor | undefined;

        let compilerOutput: string | undefined;
        vscode.window.visibleTextEditors.forEach(editor => {
            console.log('Document:')
            console.log(editor.document.fileName.toLowerCase())
            console.log(editor.document.uri.fsPath.toLowerCase())
            if (editor.document.fileName.toLowerCase().includes('compiler') &&
                editor.document.fileName.toLowerCase().includes('latex')) {
                compilerPanel = editor;
            }
        });

        if (compilerPanel) {
            compilerOutput = compilerPanel.document.getText();
        } else {
            console.log('Compiler/Output panel not found.');
        }
        
        if (compilerOutput) {
            console.log('Compiler output found. Executing command.');
            vscode.commands.executeCommand('continue.focusContinueInputWithText', compilerOutput);
        } else {
            console.log('No compiler output found.');
        }
    }, 1000);
}
