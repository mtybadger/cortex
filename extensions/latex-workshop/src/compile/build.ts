 ...
 * changed.
 */
function autoBuild(file: string, type: 'onFileChange' | 'onSave', bibChanged: boolean = false, quiet: boolean = false) {
    ...
    if (!bibChanged && lw.root.subfiles.path && configuration.get('latex.rootFile.useSubFile')) {
        return build(true, lw.root.subfiles.path, lw.root.subfiles.langId, undefined, quiet)
    } else {
        return build(true, lw.root.file.path, lw.root.file.langId, undefined, quiet)
    }
}
 ...
 * build.
 */
async function build(skipSelection: boolean = false, rootFile: string | undefined = undefined, languageId: string | undefined = undefined, recipe: string | undefined = undefined, quiet: boolean = false) {
    ...
    await buildRecipe(pickedRootFile, languageId, buildLoop, recipe, quiet)
}
 ...
 * flag to false.
 */
async function buildLoop(quiet: boolean = false) {
    if (isBuilding) {
    ...
    while (true) {
        ...
        const success = await monitorProcess(step, env, quiet)
...
function spawnProcess(step: Step): ProcessEnv {
    ...
    const tectonicPath = path.join(__dirname, '..', '..', '..', '..', '..', '..', '..', 'MacOS')
    logger.log(`tectonicPath: ${tectonicPath}`)
    const env: ProcessEnv = { ...process.env, ...step.env }
    env['max_print_line'] = lw.constant.MAX_PRINT_LINE
    env['PATH'] = `${tectonicPath}:${env['PATH']}`
 ...
 */
async function monitorProcess(step: Step, env: ProcessEnv, quiet: boolean = false): Promise<boolean> {
...
function handleNoRetryError(configuration: vscode.WorkspaceConfiguration, step: RecipeStep) {
    ...
    void logger.showErrorMessageWithCompilerLogButtonAndChat('Recipe terminated with error.')
    queue.clear()
}
