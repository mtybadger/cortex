...
export function togglePreview() {
    logger.log('TOGGLE PREVIEW command invoked.')
    const configuration = vscode.workspace.getConfiguration('latex-workshop', lw.root.getWorkspace())
    const previewEnabled = configuration.get('view.livePreview.enabled') as boolean
    if (previewEnabled) {
        configuration.update('view.livePreview.enabled', false)
        vscode.window.showInformationMessage('Live Preview turned off')
    } else {
        configuration.update('view.livePreview.enabled', true)
        vscode.window.showInformationMessage('Live Preview turned on')
    }
}
...
export async function actions() {
    ...
    return vscode.commands.executeCommand('latex-workshop.compilerlog').then(() => vscode.commands.executeCommand('workbench.action.focusActiveEditorGroup'))
}
