...
async function buildCommandTree(): Promise<LaTeXCommand[]> {
    ...
    const previewCommand = new LaTeXCommand(await lw.language.getLocaleString('activity.togglePreview'), {command: 'latex-workshop.togglePreview'}, 'eye')
    node = buildNode(previewCommand, [])
    commands.push(node)
