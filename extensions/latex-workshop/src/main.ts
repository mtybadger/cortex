...
export function activate(extensionContext: vscode.ExtensionContext) {
    ...
    extensionContext.subscriptions.push(vscode.workspace.onDidOpenTextDocument(async (document: vscode.TextDocument) => {
        if (lw.file.hasTeXLangId(document.languageId)) {
            logger.log(`LaTeX document opened: ${document.fileName}`)
            const configuration = vscode.workspace.getConfiguration('latex-workshop')
            if (configuration.get('view.livePreview.enabled')) {
                await lw.root.find()
    ...
    extensionContext.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(async (e: vscode.TextEditor | undefined) => {
        ...
        if (vscode.window.visibleTextEditors.filter(editor => lw.file.hasTeXLangId(editor.document.languageId)).length > 0) {
            ...
            if (!isLaTeXActive && configuration.get('view.livePreview.enabled')) {
                void vscode.commands.executeCommand('latex-workshop.build')
                void vscode.commands.executeCommand('latex-workshop.view')
            }
            isLaTeXActive = true
    ...
    extensionContext.subscriptions.push(vscode.workspace.onDidChangeTextDocument((e: vscode.TextDocumentChangeEvent) => {
        ...
        if (configuration.get('view.livePreview.enabled')) {
            ...
            debounceTimeout = setTimeout(() => {
                void lw.compile.autoBuild(e.document.fileName, 'onFileChange', false, true)
...
function registerLatexWorkshopCommands(extensionContext: vscode.ExtensionContext) {
    extensionContext.subscriptions.push(
        ...
        vscode.commands.registerCommand('latex-workshop.togglePreview', () => lw.commands.togglePreview()),
